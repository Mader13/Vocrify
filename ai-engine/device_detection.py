...
def detect_cuda() -> Optional[DeviceInfo]:
    ...
    try:
        ...
        return DeviceInfo(
            type=DeviceType.CUDA.value,
            name=f"NVIDIA GPU (CUDA) - Error: {str(e)}",
            available=False,
        )
...
def detect_mps() -> Optional[DeviceInfo]:
    ...
    try:
        ...
        # MPS is available
        # Note: MPS doesn't provide direct memory info like CUDA
        return DeviceInfo(
            type=DeviceType.MPS.value,
            name="Apple Silicon GPU",
            available=True,
            memory_mb=None,  # Unified memory, not directly queryable
        )
    except ImportError:
        return DeviceInfo(
            type=DeviceType.MPS.value,
        ...
        return DeviceInfo(
            type=DeviceType.MPS.value,
            name=f"Apple Silicon (MPS) - Error: {str(e)}",
            available=False,
        )

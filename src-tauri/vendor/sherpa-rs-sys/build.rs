    ...
    for entry in glob(pattern.to_str().unwrap()).unwrap() {
        match entry {
            Ok(path) => {
                ...
                // cargs is a CLI helper, not needed for the C API runtime.
                if stem_str == "cargs" {
...
fn main() {
    ...
    if is_dynamic {
        ...
        // Keep runtime artifacts minimal and avoid pulling in CLI-only cargs DLLs.
        libs_assets.retain(|path| {
            ...
            !(file == "cargs.dll"
                || file == "libcargs.so"
                || file == "libcargs.dylib"
                || file.starts_with("cargs.")
                || file.starts_with("libcargs."))
        });
        ...
        for dir in [
            ...
            for stale in ["cargs.dll", "libcargs.so", "libcargs.dylib"] {

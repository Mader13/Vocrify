...
impl DistTable {
    fn new(content: &str) -> Self {
        ...
        for value in table.targets.values_mut() {
            // expand target-specific url with {tag}
            if let Some(url_value) = value.get("url") {
                let url_value = url_value.as_str().unwrap();
                value["url"] = Value::String(url_value.replace("{tag}", &table.tag));
            }
    ...
    pub fn get(&self, target: &str, is_dynamic: &mut bool) -> Option<Dist> {
        ...
        let base_url = target_dist
            .get("url")
            .and_then(|value| value.as_str())
            .unwrap_or(&self.url);
        let url = base_url.replace("{archive}", archive);
